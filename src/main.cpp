...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    int64_t nSubsidy = 50 * COIN;			//Block fixed
			...
			else if(nBestHeight <= 300){     // F - 1
				nSubsidy = 100 * COIN;
			}
			else if(nBestHeight <= 10000){   // F - 2
				nSubsidy = 90 * COIN;
			}
			else if(nBestHeight <= 50000){  //  F - 3
				nSubsidy = 80 * COIN;
			}
			else if(nBestHeight <= 250000){ //  F - 4
				nSubsidy = 70 * COIN;
			}
			else if(nBestHeight <= 450000){ //  F - 5
				nSubsidy = 60 * COIN;
			}
			else if(nBestHeight <= 450500){ //  F - SB
				nSubsidy = 1000 * COIN;
			}
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    int64_t nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8);
	  ...
	  if(nBestHeight <= 140){			   //no POS
		  nSubsidy = nSubsidy * 0; 			
	  }
	  else if(nBestHeight <= 1000){       //F - = Estabilização da rede
				nSubsidy = nSubsidy * 195;
	  }
	  else if(nBestHeight <= 10000){       //F - 0
				nSubsidy = nSubsidy * 285;
	  }
	  else if(nBestHeight <= 50000){       //F - 1 
				nSubsidy = nSubsidy * 300;
	  }
	  else if(nBestHeight <= 250000){      //F - 2 
				nSubsidy = nSubsidy * 320;
	  }
	  else if(nBestHeight <= 450000){      //F - 3
				nSubsidy = nSubsidy * 305;
	  }
	  else if(nBestHeight <= 450500){      //BB - Bonus Block
				nSubsidy = nSubsidy * 500;
	  }
	  else if(nBestHeight <= 500000){      //F - 4 
				nSubsidy = nSubsidy * 250;
	  }
	  else if(nBestHeight > 500000){ 	   //END / INF
				nSubsidy = nSubsidy * 300;
	  }
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "MustacheCoin VIX_BTC 2020";
        CTransaction txNew;
        txNew.nTime = 1605810064;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1605810064;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 4130840 : 4130840;
        ...
        assert(block.hashMerkleRoot == uint256("0x8a697a10ae2cb145bea8b85b557f39f5e35364d10a2690463415faf329fbd11b"));
